...
public class ShareCoordinatorShard implements CoordinatorShard<CoordinatorRecord> {
    ...
    private final SharePartitionOffsetManager offsetManager;
    ...
    ShareCoordinatorShard(
        ...
        this.offsetManager = new SharePartitionOffsetManager(snapshotRegistry);
    }
    ...
    private void handleShareSnapshot(ShareSnapshotKey key, ShareSnapshotValue value, long offset) {
        ...
        offsetManager.updateState(mapKey, offset);
    }
    ...
    public Optional<Long> maybePrunePartitions() {
        System.err.println("smjn######### maybePrunePartitions:");
        long off = offsetManager.lastRedundant().get();
        if (off == 0) {
            return Optional.empty();
        }
        System.err.println("smjn######### maybePrunePartitions last redundant:" + off);
